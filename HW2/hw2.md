...
- 作图如下所示。蓝色线条是标准值，红色是neville差值得到的结果。
...
- 作图如下所示。蓝色线条是标准值，红色是cbevbyshev近似得到的结果。
...
- 作图如下所示。蓝色线条是标准值，红色是样条函数差值得到的结果。
...
- 最后，为了保证有效数字和我们之前讨论时关注的精确到小数点后几位的一致性，我们简单考察和式的大小。上面的讨论我们发现，我们主要关注第三项的大小.
...
- 第二项和第三项相仿，我们直接关注第三项的计算。第三项只需求和到$|\mathbf{n}|<2$，且被积函数并不发散，那么我们将其形式写成
